...
O projeto vem composto de dois notebooks, um onde teremos a [Analise Textual Vagas de Emprego - Análise Exploratória de Dados (AED)](/PredictSalary_RegressionProblemExample_AED.ipynb) e outro [Avaliação|Exportação|Predições](/PredictSalary_RegressionProblemExample_Training_Evaluating_Models.ipynb) onde efetuaremos as predições, avaliação do modelo e geração da saida.
...
O projeto começa sendo executado, pelo notebook [Analise Textual Vagas de Emprego - Análise Exploratória de Dados (AED)](/PredictSalary_RegressionProblemExample_AED.ipynb). Onde encontram-se algumas análises além da exportação do dataset que usaremos posteriormente para as predições que estão no notebook [Avaliação,Exportação,Predições do Modelo](/PredictSalary_RegressionProblemExample_Training_Evaluating_Models.ipynb).
